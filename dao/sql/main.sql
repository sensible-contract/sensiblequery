...
/*
    基础区块数据按每2100个区块分区保存到clickhouse，
    此数据保证和最长链一致
*/

-- block
-- ================================================================
-- 区块头，分区内按区块blkid排序、索引
-- 按blkid查询时将遍历所有分区 (慢)
DROP TABLE blk;
...
-- 区块头，分区内按区块高度height排序、索引
-- 按blk height查询时可确定分区 (快)
...
-- txin
-- ================================================================
-- 交易输入列表，分区内按交易txid+inputIdx排序、索引，单条记录包括输入的各种细节
-- 仅按txid查询时将遍历所有分区，查询需附带height。可配合tx_height表查询
...
-- 交易输入的outpoint列表，分区内按outpoint txid+idx排序、索引
-- 用于查询某txo被哪个tx花费，需遍历所有分区
...
-- txout
-- ================================================================
-- 交易输出列表，分区内按交易txid+outputIdx排序、索引，单条记录包括输出的各种细节
-- 仅按txid查询时将遍历所有分区，查询需附带height，可配合tx_height表查询
DROP TABLE txout;
